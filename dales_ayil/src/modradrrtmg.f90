...
! ==============================================================================;
  ...
  ! ============================================================================!
      ...
      use modfields, only: thl0,ql0,qt0,exnf,sv0        !#sb3
      use modsurfdata, only: thls,ps
      use modmicrodata, only : Nc_0,sig_g,imicro,imicro_bulk3      !#sb3
     ...
     ! tabs(:,:) = 0.;
      ...
      if(imicro.eq.imicro_bulk3) then ! #sb3 START
       ...
       im=i-1
        do k=1,kmax
          qv_slice (im,k)=max((qt0(i,j,k) - sv0(i,j,k,iq_cl)),1e-18) !avoid RRTMG reading negative initial values 
          qcl_slice(im,k)= sv0(i,j,k,iq_cl) ! +sv0(i,j,k,iq_hr)
          qci_slice(im,k)= sv0(i,j,k,iq_ci)+sv0(i,j,k,iq_hs) ! +sv0(i,j,k,iq_gr)
