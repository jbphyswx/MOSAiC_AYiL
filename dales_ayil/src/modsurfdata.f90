...
SAVE
  ...
  real, parameter   :: minttab = 150.4      !< minimum of tabulated temperatures
  ...
  ! Ice curface flag
  logical           :: l_surfice   = .false.  !< Flag for surface saturated with respect to ice
  logical           :: l_surficefrac = .false.  !< Flag to use sea ice fraction to calculate surface saturation
  ! Reduction of qsat over sea water due to salinity (currently only active for isurf<=2)
  !   LHF ~ (f_sal - RH_SST) with RH_SST = qsat(ps,t_skin)
  real              :: f_sal = 1.       !<  1 : no effect (default). Sea water: 0.981
  ...
  real              :: seaicefrct = 0   !<  Sea ice fraction
  real              :: thls_ocean  = -1 !<  Surface liquid water potential temperature over open water [K]
  real              :: thls_seaice = -1 !<  Surface liquid water potential temperature over sea ice    [K]
  ...
  integer, parameter:: mpatch                      = 48 !<  Maximum amount of patches that can be defined in each direction
  logical           :: lhetero                 = .false.!<  Switch to apply heterogeneous surfaces using surface.inp.xxx
  logical           :: larcticstab             = .false.!< Switch to use stability functions by Grachev et al 2007 based on SHEBA
  ...
  integer           :: ice_landtype                = -1 !< Number of ice type (for qicesurf calculation)
