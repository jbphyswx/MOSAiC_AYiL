...
!
  ...
  logical :: l_sb_classic      = .true.  &  !< .true.  ! whether to use classis S&B setting 
            ...
            ,l_setccn          = .false. &  !< whether to set CCN by constant concentration per M
            ,l_setinp          = .false. &  !< whether to set INP values instead of using loaded ones
            ,l_corr_neg_qt     = .true.  &  !< whether to adjust qt and thlp in correction for negative hydrometeor updates
            ,l_icenucle        = .true.  &  !< whether to do ice nucleation
            ,l_hetfreeze       = .true.  &  !< whether to do heterogeneous freezing of droplets and rain
            ,l_icemulti        = .true.  &  !< whether to do ise multiplication
  ...
  logical :: l_clouds_init     = .false. &  !< whether clouds were already initialised 
            ,l_iceclouds_init  = .false. &  !< whether ice clouds were already initialised 
  ...
  logical :: l_sb_lim_aggr     = .true.  &  !< whether to start with snow aggregation only after specific size
            ...
            ,l_c_inp           = .true.  &  !< whether to use constant coefficient INP
            ...
            ,l_sb_inuc_sat     = .false. &  !< ice nucleation:   whether include q_ci in calculation supersaturation with respect to ice
            ,l_sb_inuc_expl    = .false. &  !< ice nucleation:   whether explicit nucleation  
            ,l_sb_cl_noinuc    = .false. &  !< ice nucleation:   whether to turn OFF ice nucelation inside liquid clouds
            ,l_sb_reisner      = .true.     !< whether to use Reisner correction in ice nucleation
            
  
  ! setting for statistics          
  logical :: l_bulk3stat      = .false.  &  !< whether to produce averaged statistics
            ,l_hdump          = .false.  &  !< whether to do hydrometeor dumps 
            ,l_hcross         = .false.  &  !< whether to do hydrometeor crossesctions 
            ,l_hbinary        = .false.  &  !<- whether to use binary
            ,l_hdiracc        = .false.
  
  integer :: crossplane       = 2        &  !< Location of the xz crosssection
            ,crossortho       = 2        &  !< Location of the yz crosssection
            ,klow             = 1        &  !< Location of the lowest output level for hydrometeor dumps
            ,khigh            = 2           !< Location of the highest output level for hydrometeor dumps
            
  integer, dimension(100) ::  crossheight   !< Locations of the xy crosssections
  
  real    ::  dtav_stat       = 3600.0   &   !< sampling interval for profiles 
             ,timeav_stat     = 3600.0   &   !< averaging interval for profiles
             ,dtav_hcross     = 3600.0   &   !< sampling interval for hydrometeor crossesctions
             ,dtav_hdump      = 3600.0       !< sampling interval for hydrometeor fielddumps
   
  ! default number and size values 
  real,parameter  :: def_Nc0           = 70.0e6   &  !<- proposed number of droplet in namelist
                    ,def_Nci0          = 100.0    &  !<- default ice crystal number concentration
                    ,def_xc0_min       = 4.2e-15  &  !<- xcmin  min mean mass of cloud water
                    ,def_tlimhetfreeze = 238.0    &  !< max heterogenous freezing at this temperature
                    ,def_Nccn0         = 100.0e6     !<- proposed initial number of cc
                    
  real    ::  Nc0             = def_Nc0           &  !<- proposed number of droplet in namelist
             ,nci0            = def_nci0          &  !<- proposed ice crystal number concentration
             ,xc0_min         = def_xc0_min       &  !<- xcmin  min mean mass of cloud water
             ,tlimhetfreeze   = def_tlimhetfreeze &  !< max heterogenous freezing at this temperature
             ,Nccn0           = def_Nccn0            !<- proposed initial number of cc
  ...
  integer ::  in_hr           =  1       &  ! |<  in_hr : number content [     kg^{-1}] for rain, 
             ...
             ,in_in           =  4       &  ! |<  in_in : number content [ kg^{-1}] kg^{-1}] of ice nucleating particles
  ...
  integer, parameter ::  sb3nsv          = 12          ! |< minimum number of scalar variables that is needed for the scheme
...
! adding values for setting of microphysiscs
  ...
  real, parameter ::  N_M92       = 1.0e3      & !< N in M92 ice nucleation scheme
                     ...
                     ,a2_R98      = 10.0         !< by order of magnitude above 
  ...
  real,allocatable, dimension(:,:,:) ::  &
     dn_act_a       &  !< dummy action variable for number of some A
    ,dn_act_b       &  !< dummy action variable for number of some B
    ,dq_act_a       &  !< dummy action variable for mass of some A
    ,dq_act_b       &  !< dummy action variable for mass of some B
    ,inter_ab       &  !< dummy interaction variable for some (A,B)
    ,nspl           &  !< dummy variable for number in split processes
    ,qspl              !< dummy variable for mass in split processes
