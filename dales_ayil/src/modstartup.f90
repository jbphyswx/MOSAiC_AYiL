...
contains
  subroutine startup(path)
      ...
      !      Roel Neggers    Uni Koeln    20190916                      |
      !-----------------------------------------------------------------|
    ...
    use modglobal,         only : version,initglobal,iexpnr, ltotruntime, runtime, dtmax, dtav_glob,timeav_glob,&
                                  ...
                                  lmoist,lcoriol,lpressgrad,lbuoy,ladvection,igrw_damp,geodamptime,lmomsubs,cu, cv,ifnamopt,fname_options,llsadv,&
    ...
    namelist/PHYSICS/ &
        ...
        lcoriol,lpressgrad,lbuoy,ladvection,igrw_damp,geodamptime,lmomsubs,ltimedep,ltimedepuv,ltimedepsv,&
  ...
  !broadcast namelists
    ...
    call MPI_BCAST(lbuoy      ,1,MPI_LOGICAL,0,commwrld,mpierr)
    call MPI_BCAST(ladvection ,1,MPI_LOGICAL,0,commwrld,mpierr)
  ...
  !isurf
    if (myid == 0) then
      ...
      case(2,10)
      case(3:4)
        if (wtsurf <-1e10 .and. .not.ltestbed)  stop 'wtsurf not set'
        if (wqsurf <-1e10 .and. .not.ltestbed)  stop 'wqsurf not set'
      case default
  ...
  subroutine readinitfiles
    ...
    real, allocatable :: height(:), th0av(:), ampl
    ...
    !-----------------------------------------------------------------
      ...
      if (myid==0) then
        ...
        if (ltestbed) then
          ...
          write(*,*) 'readinitfiles: testbed mode: initial profiles and some sfc properties obtained from scm_in.nc'
          ...
          wtsurf     = tb_wts(1)
          wqsurf     = tb_wqs(1)
          seaicefrct  = tb_seaicefrct (1)
          thls_ocean  = tb_thls_ocean (1)
          thls_seaice = tb_thls_seaice(1)


          write (6,*) '  modstartup: readinitfiles: wtsurf, wqsurf =', wtsurf, wqsurf, thls, ps, e12min
          
        else

          write(*,*) 'readinitfiles: initial profiles read from prof.inp'
          ...
          do k=1,kmax
            ...
            e12prof(k) = e12min+exp(-0.02*zf(k))  ! e12prof(k) = e12min      !RN warning: zero tke causes thermocrash near surface
        ...
        write (6,*) 'modstartup: initial profiles after reading:'
    ...
    !---------------------------------------------------------------
      ...
      do k = 1,krand
        ...
        ampl = min( randqt, 0.1* qtprof(k) )    !RN fix to prevent exploding randomnization in qt profile above tropopause: limit to 10% of qt value
        call randomnize(qtm ,k,ampl ,irandom,ih,jh,negval)
        call randomnize(qt0 ,k,ampl ,irandom,ih,jh,negval)
      end do
    ...
    if(myid==0)then
      ...
      if (ltestbed) then
          ...
          do k=1,kmax
            ...
            dthldtls(k) = tb_thladv(1,k)
            dudtls  (k) = tb_uadv(1,k)    
            dvdtls  (k) = tb_vadv(1,k)
          end do
...
! MPI broadcast variables read in
    ...
    call MPI_BCAST(dthldtls ,kmax,MY_REAL   ,0,comm3d,mpierr)
    call MPI_BCAST(dudtls   ,kmax,MY_REAL   ,0,comm3d,mpierr)
    call MPI_BCAST(dvdtls   ,kmax,MY_REAL   ,0,comm3d,mpierr)
