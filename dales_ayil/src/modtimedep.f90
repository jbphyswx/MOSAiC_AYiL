...
! switches for timedependent surface fluxes and large scale forcings
  ...
  real, allocatable     :: z0mavt   (:)
  real, allocatable     :: z0havt   (:)
  real, allocatable     :: albedoavt(:)
  real, allocatable     :: seaicefrctt (:)
  real, allocatable     :: thls_oceant (:)
  real, allocatable     :: thls_seaicet(:)
  real, allocatable     :: Qnetavt  (:)
  
  real, allocatable     :: xlatt    (:)
  real, allocatable     :: xlont    (:)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine inittimedep
    ...
    use modglobal, only :cexpnr,k1,kmax,ifinput,runtime,zf,xlat,xlon
    use modsurfdata,only :ps,qts,wqsurf,wtsurf,thls, Qnetav, lhetero  &
                         ,z0mav, z0hav, albedoav                      &
                         ,l_surficefrac, seaicefrct, thls_ocean, thls_seaice
    use modtimedepsv, only : inittimedepsv

    use modtestbed,   only : ltestbed,ntnudge                            &
                            ,tb_time,tb_ps,tb_qts,tb_thls,tb_wqs,tb_wts  &
                            ,tb_lat,tb_lon,tb_w,tb_ug,tb_vg              &
                            ,tb_uadv,tb_vadv,tb_qtadv,tb_thladv,tb_Qnet  &
                            ,tb_z0m, tb_z0h, tb_alb                      &
    ...
    allocate(qtst     (kflux))
    allocate(pst      (kflux))
    allocate(z0mavt   (kflux))
    allocate(z0havt   (kflux))
    allocate(albedoavt(kflux))
    allocate(seaicefrctt  (kflux))
    allocate(thls_oceant  (kflux))
    allocate(thls_seaicet (kflux))
    allocate(Qnetavt  (kflux))
    allocate(xlatt    (kflux))
    allocate(xlont    (kflux))
    ...
    z0havt   = z0hav
    albedoavt= albedoav
    seaicefrctt  = seaicefrct
    thls_oceant  = thls_ocean
    thls_seaicet = thls_seaice
    Qnetavt  = Qnetav
    xlatt    = xlat
    xlont    = xlon
    ...
    if (myid==0) then
      ...
      if (ltestbed) then
        ...
        z0havt   = tb_z0h
        albedoavt= tb_alb
        seaicefrctt  = tb_seaicefrct
        thls_oceant  = tb_thls_ocean
        thls_seaicet = tb_thls_seaice
        Qnetavt  = tb_Qnet
        xlatt    = tb_lat
        xlont    = tb_lon
...
!      end do
    ...
    call MPI_BCAST(z0mavt           ,kflux,MY_REAL,0,comm3d,mpierr)
    call MPI_BCAST(z0havt           ,kflux,MY_REAL,0,comm3d,mpierr)
    call MPI_BCAST(albedoavt        ,kflux,MY_REAL,0,comm3d,mpierr)
    
    call MPI_BCAST(xlatt            ,kflux,MY_REAL,0,comm3d,mpierr)
    call MPI_BCAST(xlont            ,kflux,MY_REAL,0,comm3d,mpierr)
    ...
    if(l_surficefrac) then 
      call MPI_BCAST(seaicefrctt ,kflux,MY_REAL,0,comm3d,mpierr)
      call MPI_BCAST(thls_oceant ,kflux,MY_REAL,0,comm3d,mpierr)
      call MPI_BCAST(thls_seaicet,kflux,MY_REAL,0,comm3d,mpierr)
    end if
...
!-----------------------------------------------------------------|
  ...
  subroutine timedepsurf
    use modglobal,   only : rtimee, lmoist, xlat, xlon
...
!cstep: not necessary to provide qts in ls_flux file qts    = qtst(t)    + fac * ( qtst(t+1)    - qtst(t)     )
    z0mav  = z0mavt    (t) + fac * ( z0mavt    (t+1) - z0mavt    (t)  )
    z0hav  = z0havt    (t) + fac * ( z0havt    (t+1) - z0havt    (t)  )
    albedoav  = albedoavt    (t) + fac * ( albedoavt    (t+1) - albedoavt    (t)  )
    xlat   = xlatt  (t) + fac * ( xlatt  (t+1) - xlatt  (t)  )
    xlon   = xlont  (t) + fac * ( xlont  (t+1) - xlont  (t)  )
    !write(0,'(a,f8.2,a,f8.2)') "  timedep: xlat=",xlat," xlon=", xlon
    if (l_surficefrac) then ! update only when surface ice fraction used
      seaicefrct  = seaicefrctt (t) + fac * (seaicefrctt (t+1) - seaicefrctt (t) )
      thls_ocean  = thls_oceant (t) + fac * (thls_oceant (t+1) - thls_oceant (t) )
      thls_seaice = thls_seaicet(t) + fac * (thls_seaicet(t+1) - thls_seaicet(t) )     
    endif
  ...
  subroutine exittimedep
    ...
    deallocate(timeflux, wtsurft,wqsurft,thlst,qtst,pst,Qnetavt,xlatt,xlont)
    deallocate(z0mavt, z0havt,albedoavt,seaicefrctt,thls_oceant,thls_seaicet)
    call exittimedepsv
