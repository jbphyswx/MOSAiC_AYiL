...
!-----------------------------------------------------------------|
  ...
  use modglobal, only : i1,j1,kmax,dzh,dzf,grav,lpressgrad,lbuoy
  ...
  real :: fac_buoy         = 0.    !activator for complete buoyancy term
  real :: fac_pressgrad    = 0.    !activator for pressure gradient term (derived from prescribed geostrophic wind)
  ...
  if ( (imicro==imicro_sice).or.(imicro==imicro_bulk).or.(imicro==imicro_bin)) then
    ...
    up(i,j,k) = up(i,j,k) - fac_pressgrad*dpdxl(k)
    vp(i,j,k) = vp(i,j,k) - fac_pressgrad*dpdyl(k)
    wp(i,j,k) = wp(i,j,k) + fac_buoy * ( grav*(thv0h(i,j,k)-thvh(k))/thvh(k) - &
...
!     --------------------------------------------
  ...
  do i=2,i1
      ...
      up(i,j,1) = up(i,j,1) - fac_pressgrad*dpdxl(1)
      vp(i,j,1) = vp(i,j,1) - fac_pressgrad*dpdyl(1)
      wp(i,j,1) = 0.0
...
!     1.1 lowest model level above surface : only downward component
    ...
    if (lmomsubs) then
      do j=2,j1
        do i=2,i1
          ...
          ! moving this loop out in order to #optim ize
          !do n=1,nsv
          !  subs_sv =  0.5*whls(2)  *(sv0(i,j,2,n)-sv0(i,j,1,n)  )/dzh(2)
          !  ! svp(i,j,1,n) = svp(i,j,1,n)-subs_sv
          !  svp(i,j,1,n) = svp(i,j,1,n)+dsvdtls(1,n)-subs_sv
          !enddo
  ...
  endif !neglect effect of mean ascending on tendencies at the lowest full level
     ...
     if (lmomsubs) then
      do j=2,j1
        do i=2,i1
          ...
          !do n=1,nsv  !#optim izing by refactoring the loops
     ...
     if (lmomsubs) then
      do j=2,j1
        do i=2,i1     
          ...
          !  ! svp(i,j,k,n) = svp(i,j,k,n)-subs_sv
          !  svp(i,j,k,n) = svp(i,j,k,n)+dsvdtls(k,n)-subs_sv
          !enddo
          thlp(i,j,k) = thlp(i,j,k)-u0av(k)*dthldxls(k)-v0av(k)*dthldyls(k)-subs_thl + dthldtls(k)
          qtp (i,j,k) = qtp (i,j,k)-u0av(k)*dqtdxls (k)-v0av(k)*dqtdyls (k)-subs_qt  + dqtdtls(k)
          up  (i,j,k) = up  (i,j,k)-u0av(k)*dudxls  (k)-v0av(k)*dudyls  (k)-subs_u   + dudtls(k)
