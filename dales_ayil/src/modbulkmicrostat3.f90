...
subroutine initbulkmicrostat3
    ...
    use modstat_nc, only : lnetcdf,open_nc,define_nc,ncinfo,nctiminfo    &
                          ,writestat_dims_nc
    use modgenstat, only : idtav_stat=>idtav, itimeav_prof=>itimeav,ncid_prof=>ncid
    use modmicrodata,  only : imicro, imicro_bulk, imicro_sice, imicro_bulk3
    use modmicrodata3, only : l_bulk3stat,l_hdump, l_hcross           & ! #sb3
                             ,crossheight, crossplane, crossortho     & ! #sb3
                             ,klow, khigh                             &
                             ,dtav_stat, timeav_stat                & ! #sb3
                             ,dtav_hcross, dtav_hdump                 & ! #sb3
                             ,l_c_ccn, l_c_inp
    use modbulk3cross, only : initbulk3cross
                             
    implicit none
    integer      :: ierr

    ! read nambulkmicrostat instead of initbulkmicrostat in modbulkmicro.f90
    namelist/NAMBULK3STAT/  &
     l_bulk3stat, l_hdump, l_hcross                                &
     ,crossheight, crossplane, crossortho                          &
     ,klow, khigh                                                  &
     ,dtav_stat, timeav_stat, dtav_hcross ,dtav_hdump
    ...
    ! set default values 
    dtav_stat    = dtav_glob
    timeav_stat  = timeav_glob
    dtav_hdump   = dtav_glob
    dtav_hcross  = dtav_glob
    klow         = 1
    khigh        = kmax
    
    ! filling crossheight for start
    crossheight(1)=2
    ...
    if(myid==0)then
      ...
      read (ifnamopt,NAMBULK3STAT,iostat=ierr)
      if (ierr > 0) then
        print *, 'Problem in namoptions NAMBULK3STAT'
        print *, 'iostat error: ', ierr
        stop 'ERROR: Problem in namoptions NAMBULK3STAT'
      endif
      write(6,NAMBULK3STAT)
      close(ifnamopt)
      ...
      ! adding warnings
      if (l_bulk3stat .and. l_c_ccn) then
        write(6,*) "WARNING: using constant CCN ==> outputs for CCN are 0 "
      endif 
      if (l_bulk3stat.and. l_c_inp) then 
        write(6,*) "WARNING: using constant INP ==> outputs for INP are 0 "
      endif 
    endif 

    ! transmitting setting to other processors
    call MPI_BCAST(l_bulk3stat  ,1,MPI_LOGICAL  ,0,comm3d,mpierr)
    ...
    call MPI_BCAST(dtav_stat,1,MY_REAL  ,0,comm3d,mpierr)
    call MPI_BCAST(timeav_stat ,1,MY_REAL  ,0,comm3d,mpierr)
    call MPI_BCAST(dtav_hcross,1,MY_REAL  ,0,comm3d,mpierr)
    call MPI_BCAST(dtav_hdump    ,1,MY_REAL  ,0,comm3d,mpierr)
    ...
    ! and now for statistical outputs
    if (.not. l_bulk3stat) return
    ! get dtav, timeav values
    dtav = dtav_stat
    timeav = timeav_stat 
    ! initiate times for statistics
    idtav = dtav/tres
    itimeav = timeav/tres
    tnext      = idtav   +btime
...
!> General routine, does the timekeeping
  subroutine bulkmicrostat3
    ...
    use modmicrodata3, only  : l_bulk3stat, l_hdump, l_hcross 
    use modbulk3cross, only  : bulk3cross
    implicit none
...
!> Performs the calculations for the tendencies etc.
  subroutine bulkmicrotend3
    ...
    use modmicrodata3, only : l_bulk3stat, q_hrp, n_hrp                     & !o: qrp, Nrp
...
!------------------------------------------------------------------------------!
  ...
  subroutine exitbulkmicrostat3
    ...
    use modmicrodata3, only: l_bulk3stat, l_hdump, l_hcross
    use modbulk3cross, only : exitbulk3cross
    implicit none
...
! 
  subroutine inithydrodump
    ...
    use modmicrodata3, only: l_hdump, dtav_hdump                 &
                            ,klow, khigh                         &
    ...
    hdump_klow = klow     ! loaded through NAMBULK3STAT
    hdump_khigh = khigh   ! loaded through NAMBULK3STAT
    !dtav already adjusted 
    
    ! --- initialisation --------------------
    hdump_idtav =dtav_hdump/tres
...
!> or use 4 byte integers?
  subroutine hydrodump
    ...
    use modmicrodata3, only : l_hdump, l_hbinary,l_hdiracc           & !  ,klow, khigh                            &  
                             ,iq_cl,iq_ci,iq_hr,iq_hs,iq_hg          &
