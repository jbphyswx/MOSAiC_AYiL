...
!> Initializes and allocates the arrays
  subroutine initbulkmicro3
    ...
    use modmpi,    only : myid,my_real,comm3d,mpi_integer   &
                         ,mpi_logical  ! #sb3
    use modtestbed, only: ltb_setccn,ltb_setinp             &  ! #sb3
                         ,tb_n_inuc, tb_a_inuc, tb_b_inuc   &
                         ,tb_n_inuc_r, tb_b_inuc_r          &
                         ,ltb_iceinit
    implicit none
    integer :: ierr
    ...
    namelist/NAMBULK3/  &
     ...
     ,l_setclouds, l_setccn ,l_setinp                        & ! flag whether to set cloud 
     ,l_corr_neg_qt                                          & ! flag whether to adjust qt and thlp in hydrometeor corrections
     ,l_icenucle, l_hetfreeze, l_icemulti                    & ! flags whether to produce ice
     ...
     ,l_sb_cl_noinuc                                         & ! flags for ice nucleation
    ...
    if(myid==0) then
      ...
      if (l_sb_inuc_expl) then 
        write(6,*)  'Warning: l_sb_inuc_expl not yet implemented'
        l_sb_inuc_expl=.false.
        write(6,*)  '  setting l_sb_inuc_expl = .false.'
        Nc0 = Nc_0
      endif      
      ! close the namelist 
      close(ifnamopt)
    ...
    if(myid.eq.0) then
     if (ltb_setinp) then
       if(l_setinp)   then 
        write(6,*) 'INFO modbulkmicro3: l_setinp=T, ice nucleation parameters from MODBULK3 namelist'
        write(6,*) 'WARNING modbulkmicro3: ice nucleation parameters loaded from scm_in.nc ignored'
       else
        write(6,*) 'INFO modbulkmicro3: ice nucleation parameters loaded from scm_in.nc'
        !-d-> code to set b_inuc_R, N_inuc_R,
        N_inuc      = tb_n_inuc
        a_inuc      = tb_a_inuc
        b_inuc      = tb_b_inuc
        N_inuc_R    = tb_n_inuc_r
        b_inuc_R    = tb_b_inuc_r
       endif
     else
       if(l_setinp)   then 
        write(6,*) 'INFO modbulkmicro3: l_setinp=T, ice nucleation parameters from MODBULK3 namelist'       
       else
        write(6,*) 'WARNING: modbulkmicro3: ice nucleation parameters not set, neither by l_setinp nor scm_in.'
        ! stop 'ERROR: missing INP parameters'
        write(6,*) 'WARNING: modbulkmicro3: using default ice nucleation parameters from M92.'
        N_inuc      = N_M92
        a_inuc      = a_M92
        b_inuc      = b_M92
        N_inuc_R    = N_R98
     ...
     if(lwarmstart) then
      ...
      l_iceclouds_init  = .true. ! in warm start, ice clouds are already there
      l_ccn_init        = .true. ! in warm start, ccn are already there
     else
      if(ltb_iceinit) then
        l_iceclouds_init  = .false.
      else
        l_iceclouds_init  = .true.
      endif
      ...
      if(l_c_ccn.and.ltb_setccn) then
          write(6,*) 'INFO modbulkmicro3: l_c_ccn = .TRUE., n_ccn from scm_in ignored.'
      endif      
      if(l_setccn)    then   ! if namelist says to initialise constant CCN
        if(l_c_ccn)   then 
          write(6,*) 'WARNING modbulkmicro3: l_c_ccn = .TRUE., l_setccn ignored'
          l_ccn_init        = .true.
        else ! not constant
          write(6,*) 'INFO modbulkmicro3: l_setccn = .TRUE., n_ccn from scm_in ignored.'
          l_ccn_init        = .false.
        endif
      else ! not l_setccn
        if(ltb_setccn) then
            write(6,*) 'INFO modbulkmicro3: using n_ccn from scm_in.'
            l_ccn_init        = .false.          
        else 
            write(6,*) 'WARNING: modbulkmicro3: n_ccn not set, neither by l_setccn nor scm_in.'       
            !stop 'ERROR: missing CCN data' 
            write(6,*) 'WARNING: using default CCN and N_c parameters from SB'
            Nc0             = def_Nc0 
            xc0_min         = def_xc0_min      
            Nccn0           = def_Nccn0  
            l_ccn_init      = .false.   ! to do the setup
        endif
      endif
     endif
    endif 
    ...
    ! send values 
     ...
     call MPI_BCAST(l_iceclouds_init,    1, MPI_LOGICAL ,0,comm3d,ierr)
     call MPI_BCAST(l_ccn_init,          1, MPI_LOGICAL ,0,comm3d,ierr)
     call MPI_BCAST(l_setccn,          1, MPI_LOGICAL ,0,comm3d,ierr)
     call MPI_BCAST(l_corr_neg_qt,     1, MPI_LOGICAL ,0,comm3d,ierr)
     call MPI_BCAST(l_icenucle,        1, MPI_LOGICAL ,0,comm3d,ierr)
     call MPI_BCAST(l_hetfreeze,       1, MPI_LOGICAL ,0,comm3d,ierr)
     call MPI_BCAST(l_icemulti,        1, MPI_LOGICAL ,0,comm3d,ierr)
     !
     ...
     call MPI_BCAST(l_sb_cl_noinuc  ,  1, MPI_LOGICAL ,0,comm3d,ierr) 
     !
     ...
     call MPI_BCAST(tlimhetfreeze,           1, MY_REAL     ,0,comm3d,ierr)
...
!     write (6,*) 'l_mur_cst  = ', l_mur_cst
   ...
   ! moved allocations
    ...
    allocate( dn_act_a      (2-ih:i1+ih,2-jh:j1+jh,k1) &  !< droplet nucleation rate  
             ,dn_act_b      (2-ih:i1+ih,2-jh:j1+jh,k1) &    !< ice nucleation rate
             ,dq_act_a      (2-ih:i1+ih,2-jh:j1+jh,k1) & 
             ,dq_act_b      (2-ih:i1+ih,2-jh:j1+jh,k1) &
             ,inter_ab      (2-ih:i1+ih,2-jh:j1+jh,k1) & 
             ,nspl          (2-ih:i1+ih,2-jh:j1+jh,k1) & 
             ,qspl          (2-ih:i1+ih,2-jh:j1+jh,k1) &
             )
...
!> Cleaning up after the run
  ...
  !*********************************************************************
    ...
    deallocate(dn_act_a,dn_act_b,dq_act_a                            &
              ,dq_act_b,inter_ab                                     &
              ,nspl,qspl                                             &
              )
...
!> Calculates the microphysical source term.
  subroutine bulkmicro3
    ...
    use modtestbed, only: ltb_setccn, ltb_iceinit  ! #sb3
...
!      !#sb3 END  
    ...
    if (.not. l_ccn_init) then
     if (l_setccn) then
      ...
      if(ltb_setccn) then
        ...
        l_ccn_init = .true.
        if(myid.eq.0) then
          write(6,*) ' modbulkmicro3: initial ccn profile from scm_in'
        endif
      endif
     endif
    endif
    if (.not. l_iceclouds_init) then
     if(ltb_iceinit) then
      call initiceclouds3       ! initialise clouds
      l_iceclouds_init = .true.  
      if (myid.eq.0) then
        write(6,*) ' modbulkmicro3: ice clouds initialised by initiceclouds3'
      endif
     endif
    endif    
  ...
  !*********************************************************************
  ! if (l_rain) then  !#sb3 removing if
! old:
...
!        end if
    ...
    ! end if   ! l_rain #sb3 removing endif
    ! old:
    !     do j=2,j1
    !     do i=2,i1
    !     do k=1,k1
  ...
  !*********************************************************************
   ...
   ! if (l_rain) then ! <- move to some other part
      ...
      ! --------------------------------------------------------------
       ...
       if (l_icenucle) call icenucle3  ! ice nucleation  ! #Bb ! #iceout  
      ...
      ! --------------------------------------------------------------
       ...
       if (l_hetfreeze) call hetfreez3        ! heterogeneous freezing! #iceout 
      ...
      ! --------------------------------------------------------------      
       ...
       if (l_hetfreeze) call rainhetfreez3        ! heterogeneous freezing! #iceout         
...
! ===============================
!  ice cloud initialisation 
! ===============================
subroutine initiceclouds3
    use modglobal, only : ih,i1,jh,j1,k1,kmax,zf
    use modfields, only : rhof, qt0, svm, sv0, svp !, qvsl, qsat  ! ,exnf,ql0, svm
    use modtestbed, only: tb_qi, tb_dci
    use modmpi,    only : myid
    implicit none
    ! real , allocatable :: 
    integer :: i,j,k
    real ::    x_min, xci, q_tocl,n_prop ! availabel water and proposed size
    real ::    invbci,acoef 
    
    ! prepare threshold values 
    x_min = x_ci_bmin ! minimal mean size of ice crystals
    ...
    !if (l_c_ccn) then
      do k=1,k1
       if (tb_qi(1,k)>0.0) then
        ! prepare number of ice crystals
        xci = acoef*tb_dci(1,k)**invbci
        n_prop = tb_qi(1,k)/(xci+eps0)
        do j=2,j1
        do i=2,i1
        ! save values to arrays
         sv0(i,j,k,in_ci) = n_prop
         svm(i,j,k,in_ci) = n_prop
         sv0(i,j,k,iq_ci) = tb_qi(1,k)
         svm(i,j,k,iq_ci) = tb_qi(1,k)        
        enddo
        enddo
       endif
      enddo   
      write(*,*) ' clouds initialised by initicecloud3 on processor ', myid !#debug

end subroutine initiceclouds3


! ===============================
!  ccn initialisation 
! ===============================
! initialises CCN concentrations
...
! initialises CCN concentrations
!  called from : bulkmicro3 if flag l_ccn_init==.false. AND ltb_setccn=.true.
! steps:
!     - set ccn fields base on loaded profile n_ccn from scm_in
   subroutine inittbccn3
    use modglobal, only : ih,i1,jh,j1,k1,kmax
    use modfields, only : rhof, svm, sv0, svp  ! ,exnf,ql0, svm
    use modtestbed, only: tb_nccn
    ! use modmpi,    only : myid
    implicit none
    ! real , allocatable :: 
    integer :: i,j,k
    real ::   n_prop  ! availabel water and proposed size
    ...
    ! Nccn_set = Nccn0 ! prescribed number of ccn
     ...
     do k=1,k1
         ! prepare number of CCNs
         n_prop = tb_nccn(1,k)/rhof(k) 
         !debug: write(6,*) 'tb_nccn(1,k) = ', tb_nccn(1,k),  'tb_nccn(2,k) = ', tb_nccn(2,k), 'tb_nccn(3,k) = ', tb_nccn(3,k)
     do j=2,j1
     do i=2,i1
        ! to get number of droplets in kg^{-1} 
        ! save values to arrays
         sv0(i,j,k,in_cc) = n_prop
         svm(i,j,k,in_cc) = n_prop        
     enddo
...
! 
  ...
  !*********************************************************************
    ...
    if (any((svm(2:i1,2:j1,1:kmax,in_cl)/delt - dq_hr_ac(2:i1,2:j1,1:kmax)/x_cl(2:i1,2:j1,1:kmax)).lt. 0.)) then
...
!! directly follows Seifert&Beheng scheme
  subroutine nucleation3
    ...
    if (l_sb_nuc_expl.and.l_sb_nuc_diff) then ! l_sb_nuc_expl AND l_sb_nuc_diff 
     ...
     if (l_c_ccn) then ! c_ccn is constant 
      if (l_sb_nuc_diff)   then   ! l_sb_nuc_diff
       if (l_sb_sat_max )   then   ! l_sb_sat_max 
        do k=2,k1-1 ! no cloud nucleation at 1 level
        do j=2,j1
        do i=2,i1
     ...
     ! run only if too m  
     if (l_sb_dbg) then 
       if (any((ssat(2:i1,2:j1,1:kmax).gt.0.0).and.((qt0(2:i1,2:j1,1:kmax)-qvsl(2:i1,2:j1,1:kmax)-svm(2:i1,2:j1,1:k1,iq_cl)-svm(2:i1,2:j1,1:k1,iq_ci))/delt - x_cnuc*dn_cl_nu(2:i1,2:j1,1:k1)).lt. 0.)) then
...
! #sb3 END
  ...
  subroutine icenucle3
    ...
    if (l_sb_inuc_expl) then ! l_sb_inuc_expl
     ...
     if (l_sb_cl_noinuc) then ! no deposition nucleation if clouds
     ! following examples form UCL-LES etc.
     if (l_sb_reisner) then ! whether to apply reisnerr correction 
      do k=1,k1
      do j=2,j1
      do i=2,i1
        ! not using qcmask condition,
        ! deposition-nucleation only outside of liquid clouds
        if((tmp0(i,j,k).lt.tmp_inuc).and.(ssice(i,j,k).gt.ssice_min).and.(.not.q_cl_mask (i,j,k))) then
         ! if conditions for nucleation, calculate it
         ! Meyers et al. (1992)
     ...
     endif !l_sb_cl_noinuc
    endif  ! l_sb_inuc_expl
     
     ! warning 
     if (l_sb_dbg) then 
      if (any((ssice(2:i1,2:j1,1:kmax).gt.0.0).and.((qt0(2:i1,2:j1,1:kmax)-qvsi(2:i1,2:j1,1:kmax)-svm(2:i1,2:j1,1:k1,iq_cl)-svm(2:i1,2:j1,1:k1,iq_ci))/delt - x_inuc*dn_ci_inu(2:i1,2:j1,1:k1)).lt. 0.)) then
      ...
      if (l_sb_classic) then  
       ...
       do i=2,i1
        if (qr_spl(i,j,k) > qrmin) then
          ...
          xr_try         = max(xrmin,min(xrmax,xr_spl(i,j,k)))
...
!            )
    ...
    qspl(2:i1,2:j1,1:k1)  = q_hg(2:i1,2:j1,1:k1)
    nspl(2:i1,2:j1,1:k1)  = n_hg(2:i1,2:j1,1:k1)
...
!              )
    ...
    do jn = 1 , n_spl ! time splitting loop
     ...
     !#t write(6,*) '  sedim step ', jn
      ...
      dq_act_a(2:i1,2:j1,1:k1) = 0.
      dn_act_a(2:i1,2:j1,1:k1) = 0.
      ! wvar(2:i1,2:j1,1:k1)    = 0.
     ...
     !#t  write(6,*) '   starting '
       ...
       do j=2,j1
       do i=2,i1
         if ( (qspl(i,j,k) > qip_min).and.(nspl(i,j,k) > 0.0) ) then
           xspl  = qspl(i,j,k)/(nspl(i,j,k)+eps0) ! JvdD Added eps0 to avoid division by zero
           xspl  = min(max(xspl,xpmin),xpmax) ! to ensure xr is within borders
           ! Dvp_spl(i,j,k) = aip*xip_spl(i,j,k)**bip          
           wfall_q = max(0.0,c_v_1 * xspl **be_ip)
           wfall_n = max(0.0,c_v_0 * xspl **be_ip)
           dq_act_a(i,j,k)   = wfall_q*qspl(i,j,k)*rhof(k)
           dn_act_a(i,j,k)   = wfall_n*nspl(i,j,k)*rhof(k)
         endif
       enddo
       enddo
       enddo
     ...
     do k=1,kmax
      do j=2,j1
      do i=2,i1
        dq_act_b(i,j,k) = qspl(i,j,k) +  (dq_act_a(i,j,k+1) - dq_act_a(i,j,k))*dt_spl/(dzf(k)*rhof(k))
        dn_act_b(i,j,k) = nspl(i,j,k) + (dn_act_a(i,j,k+1) - dn_act_a(i,j,k))*dt_spl/(dzf(k)*rhof(k))
      enddo
      enddo
      if (any(dq_act_b(2:i1,2:j1,k) .lt. 0.)) then
        ! t 
        write(6,*)'  graupel sedim too large', count(dq_act_b(2:i1,2:j1,k) .lt. 0.),myid, minval(dq_act_b), minloc(dq_act_b)
      end if
      if (any(dn_act_b(2:i1,2:j1,k) .lt. 0.)) then
        ! t 
        write(6,*)'  graupel sedim too large', count(dn_act_b(2:i1,2:j1,k) .lt. 0.),myid, minval(dn_act_b), minloc(dn_act_b) 
      end if
      do j=2,j1
      do i=2,i1
        nspl(i,j,k) = nspl(i,j,k) + &
                (dn_act_a(i,j,k+1) - dn_act_a(i,j,k))*dt_spl/(dzf(k)*rhof(k))
        qspl(i,j,k) = qspl(i,j,k) + &
                (dq_act_a(i,j,k+1) - dq_act_a(i,j,k))*dt_spl/(dzf(k)*rhof(k))
      enddo
      enddo
      ...
      if ( jn == 1 ) then
       do j=2,j1
       do i=2,i1
        precep_hg(i,j,k) = precep_hg(i,j,k)+ dq_act_a(i,j,k)/rhof(k)   ! kg kg-1 m s-1
   ...
   !       (qip_spl(2:i1,2:j1,1:k1) - q_hg(2:i1,2:j1,1:k1))/delt
    ...
    do i=2,i1
      ! tendencies
      dn_hg_se(i,j,k) = (nspl(i,j,k) - n_hg(i,j,k))/delt
      dq_hg_se(i,j,k) = (qspl(i,j,k) - q_hg(i,j,k))/delt
      ! updates 
...
! ****************************************
   ...
   ! vapour_deposit(aic,bic,alpaic,betaic,xic,dqdep)
    ...
    real, allocatable :: xip(:,:,:), nip(:,:,:)
    real    :: Si, Ginv, Gval, Fvent,  q_avail, Dvic &
              ,viic ,nrex,relax_x
    ...
    do i=2,i1
      ...
      if (q_ci_mask(i,j,k).AND.(tmp0(i,j,k).le.T_3)) then
        ...
        !
        Ginv   = (rv * tmp0(i,j,k)) / (Dv*esi) + rlvi/(Kt*tmp0(i,j,k))*(rlvi/(rv*tmp0(i,j,k)) -1.)
        ! G   (i,j,k) = (Rv * tmp0(i,j,k)) / (Dv*esl(i,j,k)) + rlv/(Kt*tmp0(i,j,k))*(rlv/(Rv*tmp0(i,j,k)) -1.)
        ! and now  **-1
        Gval     = 1./Ginv
        ! diameter        
        Dvic  = aip*xip(i,j,k)**bip
        ! terminal velocity
        viic  = alphaip*((rho0s/rhof(k))**0.5)*xip(i,j,k)**betaip
        ! N_re Reynolds numbers
        nrex  = Dvic *viic /nu_a
        ! calculating from prepared ventilation coefficients 
        Fvent  = avent1+bvent1*Sc_num**(1.0/3.0)*nrex **0.5
        !-> add there lines for the growth itself 
        ! oo: dq_dep(i,j,k) = (k_depos/rhof(k))*G(i,j,k)*Dvic(i,j,k)*F(i,j,k)*Si(i,j,k) 
        dq_ci_dep(i,j,k) = k_depos*nip(i,j,k)*Gval *Dvic *Fvent *Si 
        ...
        dq_ci_dep(i,j,k) = min(dq_ci_dep(i,j,k),max(0.0,q_avail/delt)) ! min(max(dq_ci_dep(i,j,k),-svm(i,j,k,iq_ci)/delt),max(0.0,q_avail(i,j,k)/delt))
    ...
    if (l_sb_dbg) then
     ...
     if(any(((qt0(2:i1,2:j1,1:k1)-qvsi(2:i1,2:j1,1:k1)-svm(2:i1,2:j1,1:k1,iq_cl)-svm(2:i1,2:j1,1:k1,iq_ci)-delt*dq_ci_dep(2:i1,2:j1,1:k1)).lt. 0.0 ))) then
      write(6,*) 'WARNING: ice_deposit3 too high'
      write(6,*) '  depositing more ice than available in ',count(((qt0(2:i1,2:j1,1:k1)-qvsi(2:i1,2:j1,1:k1)-svm(2:i1,2:j1,1:k1,iq_cl)-svm(2:i1,2:j1,1:k1,iq_ci)-delt*dq_ci_dep(2:i1,2:j1,1:k1)).lt. 0.0 )) 
...
! ****************************************     
    ...
    do k=1,k1
      ...
      if (qfr_mask(i,j,k).and.(tmp0(i,j,k).lt.T_3)) then
          !J_het(i,j,k) = A_het *exp( B_het*(T_3- max(tlimhetfreeze ,tmp0(i,j,k))) -1)  ! maybe only for temperatures below T_3 ?
          inter_ab(i,j,k) = A_het *exp( B_het*(T_3- max(tlimhetfreeze ,tmp0(i,j,k))) -1)  ! maybe only for temperatures below T_3 ?
      endif
    enddo
    enddo
    enddo       
    ...
    do i=2,i1
       if (q_cl_mask(i,j,k).and.(tmp0(i,j,k).lt.T_3)) then
        dn_cl_het(i,j,k) = -c_mmt_1cl *n_cl(i,j,k) * x_cl(i,j,k)* inter_ab(i,j,k)
        dq_cl_het(i,j,k) = -c_mmt_2cl *q_cl(i,j,k) * x_cl(i,j,k)* inter_ab(i,j,k)
        ! basic correction 
...
! ****************************************     
    ...
    do k=1,k1
      ! calculating for all cells with the value
      if (qfr_mask(i,j,k).and.(tmp0(i,j,k).lt.T_3)) then
          J_het(i,j,k) = A_het *exp( B_het*(T_3- max(tlimhetfreeze ,tmp0(i,j,k))) -1)
      endif
    enddo
    enddo
    enddo       
    ...
    do i=2,i1
     ...
     if (q_cl_mask(i,j,k)) then
       if (tmpj(i,j,k)>tmp_lim1_Cf02) then
           J_hom(i,j,k) = 1.0e3*exp(C_Cf02+B_Cf02*(tmpj(i,j,k)+CC_Cf02))  ! becuase original J_hom per g-1 s-1
       else
         if(tmpj(i,j,k)<tmp_lim2_Cf02) then
           J_hom(i,j,k) = 1.0e3*expC_30
         else
           J_hom(i,j,k) = 1.0e3*exp(C_20_Cf02                 &
...
!     enddo  
        ...
        if ( qcol_mask(i,j,k)) then 
          ...
          if( D_b(i,j,k).gt.D_i_a ) then
            if( D_b(i,j,k).gt.D_i_b ) then
              ...
              inter_ab(i,j,k) = E_coli*E_stick           
            else
              E_stick = c_E_o_s*exp(B_stick *(tmp0(i,j,k)+stick_off)) 
              E_stick = min(c_E_o_s,E_stick)
              inter_ab(i,j,k) = (E_coli*E_stick /dif_D_10)* (D_b(i,j,k)- D_i_a)    
            endif            
          endif 
        endif 
      enddo
    ...
    do i=2,i1
      if(qcol_mask(i,j,k)) then 
       !
        dn_act_a(i,j,k) = -rhof(k)*(pi/4)*inter_ab(i,j,k)            &
                   *n_ci(i,j,k)**2 *dlt_0aa*D_b(i,j,k)**2        &
                   *(th_0aa *v_b(i,j,k)**2+2*sigma_b**2)**0.5
        !      
        dq_act_a(i,j,k) = - rhof(k)*(pi/4)*inter_ab(i,j,k)           &
             * dlt_1aa*n_ci(i,j,k)*q_ci(i,j,k)*D_b(i,j,k)**2     &
             * ( th_1aa*v_b(i,j,k)**2+2*sigma_b**2)**0.5 
        !
        !t write(6,*) '  update at (i,j,k) ', i, j, k  
        !t write(6,*) '  dn/dt_ice = ',dn_act_a(i,j,k), ' dq/dt_ice = ',dq_act_a(i,j,k),'  E_stick = ',E_stick(i,j,k)
        !t write(6,*) '  update dq_s/sdt = ',q_hsp(i,j,k)  ,' new update dq_s/sdt =', q_hsp(i,j,k) - dq_act_a (i,j,k)
        !
      endif
    enddo
    enddo
    ...
    ! -- outputs
    dq_ci_col_iis(2:i1,2:j1,1:k1) = dq_act_a(2:i1,2:j1,1:k1)
    dn_ci_col_iis(2:i1,2:j1,1:k1) = dn_act_a(2:i1,2:j1,1:k1)
...
!      do i=2,i1
!      do k=1,k1       
!        if(svm(i,j,k,iq_ci)+delt*dq_act_a(i,j,k) .lt. 0) then
!         write(6,*) '  aggregation rate at (i,j,k)',i,j,k
!         write(6,*) '  dn/dt_ice = ',dn_act_a(i,j,k), ' dq/dt_ice = ',dq_act_a(i,j,k)
...
! ****************************************     
    ...
    real    :: sigma_a, sigma_b ,E_coli              &
               ...
               ,th_0a, th_0ab, th_0b                 &
               ,th_1ab, th_1b            
    real    :: dif_D_10, ntest,  k_enhm   ! qtest,
    real, allocatable, dimension(:,:,:) :: D_a, D_b, v_a, v_b ! <- later move outside of this subroutine
    real, allocatable, dimension(:,:,:) :: E_ab, E_stick, dn_col_b, dq_col_a
    logical ,allocatable :: qcol_mask(:,:,:)
...
!     enddo    
    ...
    do i=2,i1 
      ...
      else !(qcol_mask(i,j,k).and.(tmp0(i,j,k).gt.T_3)) then
...
!     enddo       
    ...
    do j=2,j1
    do i=2,i1
     if ( q_ci_mask(i,j,k).and.q_cl_mask(i,j,k)) then
    ...
    do j=2,j1
    do i=2,i1
     if ( q_hs_mask(i,j,k).and.q_cl_mask(i,j,k)) then
...
! ***************************************************************   
   subroutine evapmelting3
    ...
    ! ice
    call sb_evmelt3(aven_0i,aven_1i,bven_0i,bven_1i,x_ci_bmin,qicemin    & 
    ...
    ! snow 
    call sb_evmelt3(aven_0s,aven_1s,bven_0s,bven_1s,x_hs_bmin,qsnowmin   & 
    ...
    !     
    ! graupel
    call sb_evmelt3(aven_0g,aven_1g,bven_0g,bven_1g,x_hg_bmin,qgrmin     &
...
!
! ***************************************************************    
  subroutine sb_evmelt3(avent0,avent1,bvent0,bvent1,x_bmin,q_emin     &
           ,n_e,q_e,x_e,D_e,v_e,dq_me,dn_me,dq_ev,dn_ev)
   ! vapour_deposit(aic,bic,alpaic,betaic,xic,dqdep)
    ...
    real, intent(in)  :: q_emin            ! minimal mass concentration
    ...
    !                       ,g_me(:,:,:),g_ev(:,:,:),x_er(:,:,:)    &
    !                       ,me_q(:,:,:), me_n(:,:,:)
    real                 :: S, nrex, f0, f1, g_me, g_ev, x_er         &
                           ,me_q, me_n
    ...
    do i=2,i1
     ...
     if (( tmp0(i,j,k).gt.T_3 ).and.(q_e(i,j,k).gt.q_emin)) then 
    ...
    do i=2,i1
      ...
      if (melt_mask(i,j,k)) then
       ! calculation of the subsaturation
        S  = min(0.0,((qt0(i,j,k)-q_cl(i,j,k))/qvsl(i,j,k)- 1.0))
      ...
      ! and limiting so not removing more than available
        ...
        dq_me(i,j,k) = me_q ! min(0.0, me_q(i,j,k) - dq_ev(i,j,k)) 
        dn_me(i,j,k) = me_n ! min(0.0, me_n(i,j,k) - dn_ev(i,j,k)) 
...
! ****************************************       
   ...
   subroutine check_allupdates(flag)
     use modglobal, only : i1,j1,k1 ! ,jh, ih,
     use modfields, only : svm,svp,ql0,qvsl,qvsi,qt0     & ! sv0,
  ...
  subroutine check_ssat(flag_dbg)
    ...
    use modglobal, only : i1,j1,ih,jh,k1 !, cp, dzf,pi,kmax,,rlv
    use modfields, only : rhof, qvsl,svm,svp, qvsi, qt0,ql0,thl0,tmp0,thlp,qtp,rhof   ! ,exnf,ql0, w0,
    ! use modmpi,    only : myid
    ! -> add other needed variables 
    implicit none
   ...
   subroutine check_nan(flag_dbg,flag,proc_name)
     use modglobal, only : ih,i1,jh,j1,k1
     use modfields, only : svm,svp,ql0,qvsl,qvsi,qt0, thl0, qtp,thlp,tmp0,rhof ! sv0
   implicit none
    logical, intent (inout) ::flag_dbg,flag
    character (len=7), intent (in) :: proc_name
    real ::  ssat, ssat_ice, uncond
