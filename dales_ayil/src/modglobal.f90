...
save
      ...
      real,parameter :: es0      = 610.78           !<    * Tetens formulae for saturation vapor pressure (Murray, F. W., J. Appl. Meteorol., 6, 203-204, 1967)
      real,parameter :: at       = 17.27            !<    *   over water (e_w)
      real,parameter :: bt       = 35.86            !<    *   over water (e_w)
      real,parameter :: at_i     = 21.8745584       !<    *   over ice (e_i)
      real,parameter :: bt_i     = 7.66             !<    *   over ice (e_i)
      ...
      !Terms in NS equation - Activation switches
      logical :: lcoriol    = .true.  !<  switch for coriolis force (True = default)
      logical :: lpressgrad = .true.  !<  switch for horizontal pressure gradient force (True = default)
      logical :: lbuoy      = .true.  !<  switch for buoyancy term (True = default)
      logical :: ladvection = .true.  !<  switch for advection term (True = default)
  ...
  subroutine initglobal
    ...
    if (courant<0) then
      ...
      elseif (any(iadv_sv(1:nsv)==iadv_kappa) .or. any((/iadv_thl,iadv_qt,iadv_tke/)==iadv_kappa)) then
        courant = min(courant, 0.7)
      elseif (any(iadv_sv(1:nsv)==iadv_kappa_f) .or. any((/iadv_thl,iadv_qt,iadv_tke/)==iadv_kappa_f)) then
        courant = min(courant, 0.7)
