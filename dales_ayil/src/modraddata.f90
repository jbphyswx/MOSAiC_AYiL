...
SAVE
  ...
  logical :: lCnstZenithTime = .false.   !< Switch to disable the diurnal cycle and use constant time for Zenith angle calculation
  ...
  real :: cnstZenithTime=0.              !< constant Time for zenith angle calculation, only used when lCnstZenithTime=.true.
  real :: emissurf = 1.0                 !< surface emissivity in radiation schmees, should be within range [0.8 , 1.0]
  
  ! Options in NAMRADIATION that apply to MODRADFULL
  logical :: l_radfullice = .false.      !< use radiative effects on ice 
  logical :: l_nc_const   = .true.       !< use constant cloud drop number 
  logical :: l_nci_const  = .true.       !< use constant cloud ice crystal number 
  logical :: l_pre_const  = .true.       !< use constant effective radious for cloud water
  logical :: l_pde_const  = .true.       !< use constant effective diameter for ice
  ...
  ! parameters for MODRADFULL
  real, parameter   :: cwmksi_limit  =  1.0e-8                &   ! minimum to be considered gridcell with ice
                      ,def_pre_cval  =  5.0e-6                &   ! effective radius of cloud droplet  [m] (if l_pre_const)  #ice
                      ,def_pde_cval  = 55.0e-6                    ! effective diameter of ice crystals [m] (if l_pde_const)  #ice
                        !,
  ! additional coefficients for MODRADFULL
  real              :: pre_cval       = def_pre_cval          &   ! effective radius of cloud droplet  [m] (if l_pre_const)  #ice
                      ,pde_cval       = def_pde_cval               ! effective diameter of ice crystals [m] (if l_pde_const)  #ice
...
!     implicit none
    ...
    if (.not.lCnstZenith) then
      ...
      if (.not.lCnstZenithTime) then
            daytime= mod(time,86400.)
      else
            daytime=cnstZenithTime * 3600
      end if
