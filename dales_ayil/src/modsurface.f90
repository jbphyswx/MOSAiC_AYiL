...
!> Reads the namelists and initialises the soil.
  subroutine initsurface
    ...
    use modtestbed, only : tb_land_use, ltestbed, ltb_setsurficefrac
 ...
 character(len=1500) :: readbuffer
    ...
    if (myid==0) then
      if (ltestbed) then
        land_use = tb_land_use
      endif
    endif
    
    ! check for surface ice flags
    if(myid==0) then
     if(l_surficefrac) then 
      if(l_surfice) then
       stop "ERROR::: l_surfice and l_surficefrac can not be true at the same time"
      endif
      if(lhetero) then
       stop "ERROR::: lhetero and l_surficefrac can not be true at the same time"
      endif
      if(larcticstab) then
              write(6,*) "Notice: Using stability functions by Grachev et al 2007"
      endif
      if (ltestbed) then
       ! if testbed, then variables have to be there
       if(ltb_setsurficefrac) then
        ! this is ok
        write(6,*) "NOTICE: using seaicefrct, thls_seaice, and thls_ocean loaded in testbed"
       else 
        write(6,*) "WARNING: l_surficefrac and ltestbed both used, requires:"
        write(6,*) "        seaicefrct, thls_seaice, or thls_ocean in scm_in.nc"
        stop "ERROR: seaicefrct, thls_seaice, or thls_ocean in scm_in.nc invalid"
       endif
      else ! not testbed
        ! need to check inserted values
        if((seaicefrct<0.0).or.(seaicefrct>1.0)) then
          stop "ERROR: l_surficefrac true but seaicefrct wrong"
        endif        
        if(thls_ocean<minttab) then
          stop "ERROR: l_surficefrac true but thls_ocean wrong"
        endif
        if(thls_seaice<minttab) then
          stop  "ERROR: l_surficefrac true but thls_seaice wrong"
        endif
      endif
     else ! .not.l_surficefrac
      if(seaicefrct>0) then
        write(6,*) "WARNING: seaicefrct ignored when l_surficefrac=.FALSE."
      endif
     endif
    endif
    
    ! pass namelist variables to other processors
    ...
    call MPI_BCAST(seaicefrct   , 1, MY_REAL, 0, comm3d, mpierr)
    call MPI_BCAST(thls_ocean   , 1, MY_REAL, 0, comm3d, mpierr)
    call MPI_BCAST(thls_seaice  , 1, MY_REAL, 0, comm3d, mpierr)
    call MPI_BCAST(f_sal        , 1, MY_REAL, 0, comm3d, mpierr)
    ...
    call MPI_BCAST(l_surficefrac              ,            1, MPI_LOGICAL, 0, comm3d, mpierr)
    call MPI_BCAST(lhetero                    ,            1, MPI_LOGICAL, 0, comm3d, mpierr)
    call MPI_BCAST(larcticstab                    ,            1, MPI_LOGICAL, 0, comm3d, mpierr)
            ...
            do while (ierr == 0)
              ...
              if (ierr == 0) then                               !So no end of file is encountered
                if (readbuffer(1:1)=='#') then
                  ...
                  if ((l_surfice) .and. (landname(i) == 'ice')) then
                          if (ice_landtype .ne. -1) stop "NAMSURFACE: More than one type named ice"
                          ice_landtype = landtype(i)
                          write (*,*) "ice_type", ice_landtype
                  endif
    ...
    ! write( 6,'(a,3f10.7,f12.3,3f10.3)' ) 'initsurface: (albedoav,z0mav,z0hav,ps,thls,thls_ocean,thls_seaice)  = ', &
    !   albedoav, z0mav, z0hav, ps, thls, thls_ocean,thls_seaice
...
!> Calculates the interaction with the soil, the surface temperature and humidity, and finally the surface fluxes.
  subroutine surface
    ...
    ! write( 6,'(a,3f10.7,f12.3,3f10.3)' ) 'surface: (albedoav,z0mav,z0hav,ps,thls,thls_ocean,thls_seaice)  = ', &
    !   albedoav, z0mav, z0hav, ps, thls, thls_ocean,thls_seaice
    ...
    if(lhetero) then
      ...
      ! note: z0m and z0h not updated here in heterogeneous case
    else
      ! updating values of 2D surface fields in non-heterogeneous case
      z0m = z0mav
      z0h = z0hav
      albedo = albedoav
    endif
    ...
    elseif(isurf == 2) then
      do j = 2, j1
        do i = 2, i1
          ...
          else if (l_surficefrac) then
            tskin(i,j) = (1.-seaicefrct)*thls_ocean+seaicefrct*thls_seaice
          else 
            tskin(i,j) = thls
          endif
        end do
    ...
    if(isurf <= 2) then
      do j = 2, j1
        do i = 2, i1
          ...
          qtflux (i,j) = - ( qt0 (i,j,1) - qskin(i,j) * f_sal) / ra(i,j)    !account for reduction in qsat over sea water due to salinity
    ...
    if(isurf <= 2) then
      ...
      do j = 2, j1
        do i = 2, i1
          ...
          tsurf      = tskin(i,j) * exner
          if (tsurf>tmelt) then
            ! saturated with respect to melting surface
            es         = es0 * exp(at*(tsurf-tmelt) / (tsurf-bt))
            qsatsurf   = rd / rv * es / ps         
          else 
            ! saturted with respect to ice 
            es         = es0 * exp(at_i*(tsurf-tmelt) / (tsurf-bt_i))
            qsatsurf   = rd / rv * es / ps
          endif 
          surfwet    = ra(i,j) / (ra(i,j) + rs(i,j))
          qskin(i,j) = surfwet * qsatsurf + (1. - surfwet) * qt0(i,j,1)
      ...
      if (lhetero) then
        ...
        do j = 2, j1
          do i = 2, i1
            ...
            if (land_use(patchx,patchy) .eq. ice_landtype) then
              if (tsurf>tmelt) then
               ! saturated with respect to melting surface
               es         = es0 * exp(at*(tsurf-tmelt) / (tsurf-bt))
               qsatsurf   = rd / rv * es /  ps_patch(patchx,patchy)       
...
!> Calculate the surface humidity assuming saturation with respect to ice/water based on sea ice fraction

  subroutine qseaicefrctsurf
    use modglobal,   only : tmelt,bt,at,rd,rv,cp,es0,pref0,ijtot,i1,j1    &
                            ,esatitab,ttab, at_i, bt_i
    use modfields,   only : qt0
    !use modsurfdata, only : rs, ra
    ...
    implicit none
    real       :: exner, tsurf, tsurfi, tsurfw, qsatsurf, surfwet, es, qtsl
    integer    :: i,j, patchx, patchy
    integer    :: Npatch(xpatches,ypatches), SNpatch(xpatches,ypatches)
    integer    :: tlonr
    real       :: lqts_patch(xpatches,ypatches)
    ...
    if(isurf <= 2) then
      ...
      do j = 2, j1
        do i = 2, i1
          exner      = (ps / pref0)**(rd/cp)
          ! tsurf      = tskin(i,j) * exner
          tsurfi     = thls_seaice * exner    
          tsurfw     = thls_ocean  * exner    
          if (tsurfi>tmelt) then ! if (tsurf>tmelt) then
            ! saturated with respect to melting surface
            esi         = es0 * exp(at*(tsurfi-tmelt) / (tsurfi-bt))        
          else 
            ! saturted with respect to ice 
            esi         = es0 * exp(at_i*(tsurfi-tmelt) / (tsurfi-bt_i))
          endif           
          esw        = es0 * exp(at*(tsurfw-tmelt) / (tsurfw-bt))
          es         = seaicefrct * esi + (1-seaicefrct) * esw
          qsatsurf   = rd / rv * es / ps
          surfwet    = ra(i,j) / (ra(i,j) + rs(i,j))
          qskin(i,j) = surfwet * qsatsurf + (1. - surfwet) * qt0(i,j,1)
          qtsl       = qtsl + qskin(i,j)
  ...
  end subroutine qseaicefrctsurf
...
!> Calculates the Obukhov length iteratively.
  ...
  function psim(zeta)
    ...
    if(zeta <= 0) then
      ...
      !psim = 3. * log( (1. + 1. / x) / 2.)
    elseif (larcticstab) then
            x = (1+zeta)**(1./3.)
            am = 5.
            bm = am / 6.5
            bbm = ((1-bm)/bm)**(1./3.)
            psim = -1* 3*am / bm *(x-1) + am*bbm / (2*bm) * (2*log((x+bbm) / (1+bbm)) - log((x**2 - x*bbm + bbm**2) / (1-bbm + &
            bbm**2)) + 2*sqrt(3.)*(atan((2*x-bbm)/(sqrt(3.)*bbm)) - atan((2-bbm)/(sqrt(3.)*bbm))))
    else
  ...
  function psih(zeta)
    ...
    elseif (larcticstab) then
            ...
            psih = -1* bh / 2 * log(1+ch*zeta+zeta**2) + (-1*ah / bbh + bh*ch / (2*bbh)) * (log((2*zeta + ch - bbh) / (2*zeta + ch + &
            bbh))-log((ch-bbh)/(ch+bbh)))
    else
 ...
 function phim(zeta)
    ...
    elseif (larcticstab) then
            phim = 1 + (6.5*zeta*(1+zeta)**(1./3.)) / (1.3 + zeta)
 ...
 function phih(zeta)
    ...
    elseif (larcticstab) then
            phih = 1 + (5*zeta + 5*zeta**2) / (1+3*zeta+zeta**2)
