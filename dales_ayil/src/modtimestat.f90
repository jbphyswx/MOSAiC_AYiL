...
!NetCDF variables
 ...
 real :: SW_up_TOA_av, SW_dn_TOA_av, LW_up_TOA_av, LW_dn_TOA_av, &
         SW_up_ca_TOA_av, SW_dn_ca_TOA_av, LW_up_ca_TOA_av, LW_dn_ca_TOA_av
  ...
  subroutine inittimestat
    ...
    if(myid==0) then
      ...
      if (lnetcdf) then
        ...
        nvar0r = nvar     !+8 TOA radiative fluxes
        nvar   = nvar0r + 8
        ...
        call ncinfo(ncname( nvar0r+1,:),'SW_up_ca_TOA','SW upward flux at TOA - clear air'  ,'W/m^2','time')
        call ncinfo(ncname( nvar0r+2,:),'SW_dn_ca_TOA','SW downward flux at TOA - clear air','W/m^2','time')
        call ncinfo(ncname( nvar0r+3,:),'LW_up_ca_TOA','LW upward flux at TOA - clear air'  ,'W/m^2','time')
        call ncinfo(ncname( nvar0r+4,:),'LW_dn_ca_TOA','LW downward flux at TOA - clear air','W/m^2','time')
        call ncinfo(ncname( nvar0r+5,:),'SW_up_TOA','SW upward flux at TOA'  ,'W/m^2','time')
        call ncinfo(ncname( nvar0r+6,:),'SW_dn_TOA','SW downward flux at TOA','W/m^2','time')
        call ncinfo(ncname( nvar0r+7,:),'LW_up_TOA','LW upward flux at TOA'  ,'W/m^2','time')
        call ncinfo(ncname( nvar0r+8,:),'LW_dn_TOA','LW downward flux at TOA','W/m^2','time')
...
!
    ...
    use modraddata,only : SW_up_TOA, SW_dn_TOA, LW_up_TOA, LW_dn_TOA, &
    ...
    real :: SW_up_TOA_avl, SW_dn_TOA_avl, LW_up_TOA_avl, LW_dn_TOA_avl, &
            SW_up_ca_TOA_avl, SW_dn_ca_TOA_avl, LW_up_ca_TOA_avl, LW_dn_ca_TOA_avl
...
!     -------------------------
!     9.2 C  horizontally averaged TOA radiative fluxes
!     -------------------------

    SW_up_TOA_avl    = sum(SW_up_TOA   (2:i1,2:j1))
    SW_dn_TOA_avl    = sum(SW_dn_TOA   (2:i1,2:j1))
    LW_up_TOA_avl    = sum(LW_up_TOA   (2:i1,2:j1))
    LW_dn_TOA_avl    = sum(LW_dn_TOA   (2:i1,2:j1))
    SW_up_ca_TOA_avl = sum(SW_up_ca_TOA(2:i1,2:j1))
    SW_dn_ca_TOA_avl = sum(SW_dn_ca_TOA(2:i1,2:j1))
    LW_up_ca_TOA_avl = sum(LW_up_ca_TOA(2:i1,2:j1))
    LW_dn_ca_TOA_avl = sum(LW_dn_ca_TOA(2:i1,2:j1))

    call MPI_ALLREDUCE(SW_up_TOA_avl,SW_up_TOA_av, 1, MY_REAL, &
                       MPI_SUM, comm3d,mpierr)
    call MPI_ALLREDUCE(SW_dn_TOA_avl,SW_dn_TOA_av, 1, MY_REAL, &
                       MPI_SUM, comm3d,mpierr)
    call MPI_ALLREDUCE(LW_up_TOA_avl,LW_up_TOA_av, 1, MY_REAL, &
                       MPI_SUM, comm3d,mpierr)
    call MPI_ALLREDUCE(LW_dn_TOA_avl,LW_dn_TOA_av, 1, MY_REAL, &
                       MPI_SUM, comm3d,mpierr)
    call MPI_ALLREDUCE(SW_up_ca_TOA_avl,SW_up_ca_TOA_av, 1, MY_REAL, &
                       MPI_SUM, comm3d,mpierr)
    call MPI_ALLREDUCE(SW_dn_ca_TOA_avl,SW_dn_ca_TOA_av, 1, MY_REAL, &
                       MPI_SUM, comm3d,mpierr)
    call MPI_ALLREDUCE(LW_up_ca_TOA_avl,LW_up_ca_TOA_av, 1, MY_REAL, &
                       MPI_SUM, comm3d,mpierr)
    call MPI_ALLREDUCE(LW_dn_ca_TOA_avl,LW_dn_ca_TOA_av, 1, MY_REAL, &
                       MPI_SUM, comm3d,mpierr)

    SW_up_TOA_av    = SW_up_TOA_av    / ijtot
    SW_dn_TOA_av    = SW_dn_TOA_av    / ijtot
    LW_up_TOA_av    = LW_up_TOA_av    / ijtot
    LW_dn_TOA_av    = LW_dn_TOA_av    / ijtot
    SW_up_ca_TOA_av = SW_up_ca_TOA_av / ijtot
    SW_dn_ca_TOA_av = SW_dn_ca_TOA_av / ijtot
    LW_up_ca_TOA_av = LW_up_ca_TOA_av / ijtot
    LW_dn_ca_TOA_av = LW_dn_ca_TOA_av / ijtot
  ...
  !     ---------------------------------------
    ...
    if(myid==0)then
      ...
      if (lnetcdf) then
        ...
        vars(nvar0r+1)  = SW_up_ca_TOA_av  !'SW upward flux at TOA - clear air'  ,'W/m^2','time')
        vars(nvar0r+2)  = SW_dn_ca_TOA_av  !'SW downward flux at TOA - clear air','W/m^2','time')
        vars(nvar0r+3)  = LW_up_ca_TOA_av  !'LW upward flux at TOA - clear air'  ,'W/m^2','time')
        vars(nvar0r+4)  = LW_dn_ca_TOA_av  !'LW downward flux at TOA - clear air','W/m^2','time')
        vars(nvar0r+5)  = SW_up_TOA_av     !'SW upward flux at TOA'  ,'W/m^2','time')
        vars(nvar0r+6)  = SW_dn_TOA_av     !'SW downward flux at TOA','W/m^2','time')
        vars(nvar0r+7)  = LW_up_TOA_av     !'LW upward flux at TOA'  ,'W/m^2','time')
        vars(nvar0r+8)  = LW_dn_TOA_av     !'LW downward flux at TOA','W/m^2','time')
