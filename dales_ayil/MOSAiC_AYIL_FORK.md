# `MOSAiC_AYiL`: changes from upstream DALES

This tree is **not** vanilla [dalesteam/dales](https://github.com/dalesteam/dales). MOSAiC_AYiL ships a fork under `MOSAiC_AYiL/` for AYiL LES reproduction. **Rebuild after pulling:** `./scripts/build_dales.sh`.
...
## Other pipeline code (not in `MOSAiC_AYiL/src`)
...
Use the resulting `MOSAiC_AYiL/build/src/dales4` on HPC and locally. Do not assume a system-wide `dales4` matches this fork.
